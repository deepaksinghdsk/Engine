...
void main()
{
    ...
    color += texture(imageSampler, inUV + vec2(texelSize.x, 0.0)) * weights[1];
    color += texture(imageSampler, inUV - vec2(texelSize.x, 0.0)) * weights[1];
    color += texture(imageSampler, inUV + vec2(texelSize.x * 2.0, 0.0)) * weights[2];
    color += texture(imageSampler, inUV - vec2(texelSize.x * 2.0, 0.0)) * weights[2];
    color += texture(imageSampler, inUV + vec2(texelSize.x * 3.0, 0.0)) * weights[3];
    color += texture(imageSampler, inUV - vec2(texelSize.x * 3.0, 0.0)) * weights[3];
